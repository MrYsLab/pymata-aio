...
/* This sketch provides additional functionality to StandardFirmata.
   ...
   Stepper
   NewPing
   Read of DS18B20 one wire temp sensor
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    ...
    case STEPPER_DATA:
      ...
      break ;
    case ONEWIRE_DATA:
      for (uint8_t pin = 2; pin < TOTAL_PINS; pin++)
      {
        OneWire oneWire(pin);
        ...
        if (numSensors > 0)
        {
           devices[pin].oneWire = &oneWire;
           devices[pin].sensor = &sensors;
           devices[pin].numberOfDevices = numSensors;
    ...
    if (numSensors > 0)
    {
      ...
      sensors.requestTemperatures();
          ...
          for (uint8_t pin = 2; pin < num_pins; pin++)
          {
              if(findDevices(pin))
              {
                  setPinModeCallback(pin, PIN_MODE_ONEWIRE);
              }
          }
          break;
        }
        case DS_REQUEST_TEMPERATURE:
        {
          byte pin = argv[1];
          uint8_t index = argv[2] + (argv[3] << 7);

          double temperature, fractional, integer;
          uint8_t i_frac, i_int;
          ...
          fractional = modf(temperature, &integer);
          i_int = (int) integer;
          i_frac = (int)(fractional * 100);

          Firmata.write(START_SYSEX);
          Firmata.write(ONEWIRE_DATA);
          Firmata.write(DS_TEMPERATURE_REPLY);
          Firmata.write(pin);
          Firmata.write(index);
          Firmata.write(i_int);
          Firmata.write(i_frac);
          if( temperature < 0.0)
          {
...
uint8_t findDevices(int pin)
{
  ...
  if (ow.search(address))
  {
    ...
    // send a sysex message with pin number and address
    Firmata.write(START_SYSEX);
    Firmata.write(ONEWIRE_DATA) ;
    Firmata.write(DS_DISCOVERY_REPLY) ;
    Firmata.write(pin);
    for (uint8_t i = 0; i < 8; i++)
    {
      Firmata.write(address[i] & 0x7F);
      Firmata.write(address[i] >> 7);
    }
    Firmata.write(END_SYSEX);
    return 1;
  }
...
void printData(char * id,  int16_t data)
{
  char myArray[64] ;
